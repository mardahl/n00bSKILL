...
### E1. Inflated importance

**Words to watch:** stands/serves as, a testament/reminder to, vital/significant/crucial/pivotal/key role/moment, underscores/highlights its importance/significance, reflects broader, setting the stage for, marks/represents a shift, key turning point, evolving landscape, indelible mark
...
### E2. Name-dropping to prove importance
...
Keep a citation when it explains what the person said and where. Do not invent context for a shorter version.

### E3. Shallow analysis with -ing phrases
...
### E6. Formulaic challenges and outlook sections
...
### E7. Overused AI words

**High-frequency list:** Actually, additionally, align with, crucial, delve, emphasizing, enduring, enhance, fostering, garner, highlight (verb), interplay, intricate/intricacies, key (adjective), landscape (abstract noun), pivotal, quietly, showcase, tapestry (abstract noun), testament, underscore (verb), valuable, vibrant

One instance in isolation is not a tell. Clusters are.
...
### E13. Passive voice and missing subjects
...
### E14. Em and en dashes

The final rewrite must not contain em dashes (—) or en dashes (–), unless the writer's own sample uses them — in that case match the sample's rate. Replace with a period, comma, colon, or parentheses, or rewrite the sentence. Also check spaced dashes (` — `) and double hyphens (` -- `).
...
### E16. Lists with bold mini-headings
...
### E20. Chatbot text left in the answer
...
### E21. Knowledge-limit disclaimers and gap-fill guesses

**Words to watch:** as of [date], Up to my last training update, While specific details are limited/scarce..., based on available information, not publicly available, maintains a low profile, likely [grew up/studied/began]
...
### E24. Stacked qualifiers

**Phrases to watch:** to be fair, it's also possible, could potentially, might arguably, in some cases it may

Keep a qualifier only when the source supports it and the meaning needs it. Remove caveats that only repair an earlier overstatement. Never remove the last qualification.
...
### E27. Pretending to reveal a deeper truth
...
### E28. Announcing the next point

**Phrases to watch:** Let's dive in, let's explore, here's what you need to know, now let's look at, without further ado, quick note

A casual phrase such as "one thing that bit me" can have the same problem. Remove the announcement, not just its formal tone.
...
### E29. A heading repeated in the first sentence
...
### E30. Writing about the previous version

Documentation and comments should describe the current behavior. Mention the previous version only in change logs, release notes, migration guides and other documents about change.
...
### E31. Forced punchlines and dramatic fragments

One short sentence can add emphasis. A row of short fragments feels forced.
...
### E33. Fake-candid openings

**Phrases to watch:** Honestly?, Look, Here's the thing, The thing is, Let's be honest, Real talk — as standalone hooks or staged pauses before an ordinary point.
...
### E34. Answering objections no one raised

**Phrases to watch:** This isn't (mainly/really) about, I'm not saying/arguing/trying to, To be clear, Don't get me wrong, You could argue this differently but, Some might say... but

Watch for an unattributed statement about what the writer does not mean, especially when the topic appears nowhere else. A direct claim such as "the API is not thread-safe" is not this pattern.
...
### E35. Rejecting fake alternatives

**Phrases to watch:** A tempting option/approach would be, One might be tempted to, An obvious approach would be, You might think... but, It would be easy to just
...
- **Formal or academic words.** E7 lists specific words that AI writing overuses. Do not simplify every formal word.
...
- **Em dashes alone.** Many editors and journalists use them often. Evidence only when paired with formulaic sales-y rhythm.
...
- **Real alternatives.** Keep options that a reader may consider in a design document, tutorial, or argument. Remove only an unlikely option the text dismisses and never uses again.
